...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
   ...
   var freqfilter = freqArray.filter(sampleObj => sampleObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
   var result = samplefilter[0]
   var freqResult = freqfilter[0]
   
    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.

    var otu_ids = result.otu_ids
    var otu_labels = result.otu_labels
    var otu_sample_values = result.sample_values
    var freqvalue = freqResult.wfreq
    ...
    var yticks = otu_ids.slice(0,10).map(lamda => `OTU ${lamda}`).reverse();
